 ...
 * a 413 raised at the edge -- before Django runs at all. Camera photos here are
 * routinely 12-15 MB, so without this the upload dies before any server-side
 * compression can see it, and nothing surfaces to the user.
 *
 * Anything already under TARGET_BYTES is returned untouched, which keeps icons
 * and small PNGs (including transparent ones) exactly as they are. Only large
 * files are re-encoded, and at that size they are always photographs, so
 * flattening to JPEG is safe.
 */

// Forms here can send a cover image plus a gallery in a single request, so the
// per-file budget has to leave room for several files inside one ~4.5 MB body.
// At 1 MB each, four images plus form fields still fit.
