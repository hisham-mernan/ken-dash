...
const Pagination = ({
  ...
  return (
    <ul
      className={`flex_center gap-2 sm:gap-6  bg-white py-3 sm:py-4 px-4 sm:px-6 rounded-[10px] border border-font-light  mx-auto ${className}`}
    >
      {" "}
      <li
        ...
        {paginationRange?.map((pageNumber, index) => {
          if (pageNumber === DOTS) {
            return (
              <li
                key={index}
                className="flex_center w-9 h-9 border border-font-light  rounded-lg dots"
              >
                <svg
                  width="13"
                  height="4"
          ...
          return (
            <li
              key={index}
              role="button"
              className={`flex_center w-9 h-9 border border-font-light hover:bg-primary-2 hover:text-[#FDF9FF] transition-all ease-in-out duration-300   rounded-lg   cursor-pointer font-inter text-base font-medium ${
