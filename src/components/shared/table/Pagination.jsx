...
const Pagination = ({
  ...
  return (
    ...
    >
      {" "}
      <li
        className={`flex_center w-8 h-8 sm:w-fit sm:h-auto  sm:py-2 sm:px-3 rounded-sm border border-font-light ${
          currentPage > 1 ? " cursor-pointer" : "opacity-50 cursor-default"
        }`}
        onClick={onPrevious}
      >
        <span
      ...
      <li className="flex_center gap-3">
        {paginationRange?.map((pageNumber, index) => {
          if (pageNumber === DOTS) {
            return (
              <div
                key={index}
                className="flex_center w-8 h-8 sm:w-9 sm:h-9 border border-font-light  rounded-lg dots"
              >
                <svg
                  width="13"
                  height="4"
          ...
          return (
            <div
              key={index}
              role="button"
              className={`flex_center w-8 h-8 sm:w-9 sm:h-9 border border-font-light hover:bg-secondary-4 hover:text-[#FDF9FF] transition-all ease-in-out duration-300   rounded-lg   cursor-pointer font-inter text-base font-medium ${
                pageNumber === currentPage
                  ? "bg-secondary-4 text-[#FDF9FF] font-bold"
                  : "text-secondary-dark"
              }`}
